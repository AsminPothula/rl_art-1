...
class PaintingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Add this check right after computing unit_vector
        ...
        # Prepare next_canvas to return — do NOT overwrite self.canvas
        # Ensure canvas is in shape (1, 1, H, W)
        # Safely return canvas with shape [1, 1, H, W]
        canvas_to_return = self.canvas
        if canvas_to_return.ndim == 2:  # (H, W)
            canvas_to_return = canvas_to_return[np.newaxis, np.newaxis, :, :]
        elif canvas_to_return.ndim == 3 and canvas_to_return.shape[-1] == 1:  # (H, W, 1)
            canvas_to_return = np.transpose(canvas_to_return, (2, 0, 1))  # → (1, H, W)
            canvas_to_return = canvas_to_return[np.newaxis, :, :, :]      # → (1, 1, H, W)
        elif canvas_to_return.ndim == 3 and canvas_to_return.shape[0] == 1:  # (1, H, W)
            canvas_to_return = canvas_to_return[np.newaxis, :, :, :]      # → (1, 1, H, W)
        elif canvas_to_return.ndim == 4:
            pass  # already good

        canvas_to_return = canvas_to_return.squeeze(0)
        print("Canvas shape in env.step:", canvas_to_return.shape)
