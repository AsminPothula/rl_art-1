...
class DDPGAgent:
    ...
    def select_action(self, canvas, target_image, prev_action):
        ...
        # canvas = canvas.to(device) # already a tensor (set in train.py)
        ...
        #!canvas = torch.FloatTensor(canvas).unsqueeze(0).to(device)    # (B, H, W, C) 
        canvas = canvas.to(device)  # already a tensor from train.py
        #!target_image = torch.FloatTensor(target_image).unsqueeze(0).to(device)  # (B, H, W, C) 
        #!prev_action = torch.FloatTensor(prev_action).unsqueeze(0).to(device)  # (B, action_dim)
        
        # Handle only if passed as numpy (for test mode)
        if isinstance(target_image, np.ndarray):
            target_image = torch.from_numpy(target_image).float()
        if target_image.ndim == 3:
            target_image = target_image.unsqueeze(0)  # (C, H, W) → (1, C, H, W)
